...
def monitor(latest_predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    monitoring_df = pd.DataFrame(columns=['predictions_time', 'mean_predictions'])
    monitoring_df['predictions_time'] = latest_predictions['predictions_time'].values[0]
    monitoring_df['mean_predictions'] = latest_predictions['predictions'].mean()
